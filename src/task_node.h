...
template<typename ObjTy, typename InTy, typename OutTy>
class TaskNode {
public:
  ...
  TaskNode(ObjTy* pobj,
           ...
           : pobj(pobj), thread_name(thread_name),
  ...
  ~TaskNode() { worker_thread.join(); }

  void SetInputQueue(InQueueTy* in_queue) {
      input_queue = in_queue;
  }

  void SetOutputQueue(OutQueueTy* out_queue) {
...
template<typename ObjTy, typename InTy>
class TaskNode<ObjTy, InTy, void> {
public:
  using InQueueTy = ThreadSafeQueueWithCapacity<InTy>;
...
class TaskNode<ObjTy, void, void> {
public:
  TaskNode(ObjTy* pobj,
           const std::string& thread_name,
           const std::string& stream_name)
