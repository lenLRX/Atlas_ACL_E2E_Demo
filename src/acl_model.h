...
public:
  ...
  DeviceBuffer(void *device_buffer, size_t buffer_size, DeleteFn delete_fn)
      : host_buffer(nullptr), device_buffer(device_buffer),
        buffer_size(buffer_size), delete_fn(delete_fn) {
    if (IsDeviceMode()) {
  ...
  void *GetHostPtr() {
    if (host_buffer == nullptr && !IsDeviceMode()) {
      ...
      CHECK_ACL(aclrtMemcpy(host_buffer, buffer_size, device_buffer,
                            buffer_size, ACL_MEMCPY_DEVICE_TO_HOST));
    }
    return host_buffer;
  }
