...
aclError VPCBatchCrop::Crop(uint8_t *src_buffer, int *x1, int *y1, int *x2,
  ...
  for (int i = 0; i < batch_num; ++i) {
    CHECK_ACL(CropBatch(x1 + offset, y1 + offset, x2 + offset, y2 + offset,
                        dst_array + offset, dst_h + offset, dst_w + offset,
                        vpc_crop_batch_size));
...
aclError VPCBatchCrop::CropBatch(int *x1, int *y1, int *x2, int *y2,
                                 uint8_t **dst_array, int *dst_h, int *dst_w,
                                 int num) {
