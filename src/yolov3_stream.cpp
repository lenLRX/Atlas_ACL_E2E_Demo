...
Yolov3Model::OutTy Yolov3Model::Process(Yolov3Model::InTy bufferx2) {
  ...
  auto output_buffers = yolov3_model.Infer({std::get<1>(bufferx2), img_info_buffer});
...
Yolov3PostProcess::OutTy Yolov3PostProcess::Process(InTy input) {
  ...
  YUV420SPImage img((uint8_t*)image_buffer->GetHostPtr(), yolov3_model_size, yolov3_model_size);
...
void Yolov3StreamThread(json config) {
  ...
  DvppDecoder decoder;
  TaskNode<DvppDecoder, AVPacket, void> decoder_node(&decoder, "DvppDecoder", input_addr);
  ...
  TaskNode<VPCResizeEngine, DeviceBufferPtr, buf_tup_t>
  resize_engine_node(&resize_engine, "VPCResizeEngine", input_addr);
  ...
  Yolov3Model yolov3_model("./model/sample-yolov3_pp_416.om", model_stream);
  ...
  ThreadSafeQueueWithCapacity<Yolov3Model::OutTy> yolov3_post_input_queue(queue_size);
  ...
  ThreadSafeQueueWithCapacity<Yolov3PostProcess::OutTy> yolov3_post_output_queue(queue_size);
  post_process_node.SetOutputQueue(&yolov3_post_output_queue);
  post_process_node.Start(ctx);

  FFMPEGOutput ffmpeg_output;
  TaskNode<FFMPEGOutput, DeviceBufferPtr, void> ffmpeg_output_node(&ffmpeg_output, "FFMPEGOutput", input_addr);
  ...
  if (camera_id < 0) {
    {
      TaskNode<FFMPEGInput, void, void> ffmpeg_input_node(&ffmpeg_input, "FFMPEGInput", input_addr);
